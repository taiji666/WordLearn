...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (reviewButton) {
        reviewButton.addEventListener('click', () => {
            ...
            // document.body.classList.toggle('in-review-mode', isReviewMode); // This class can be used for global styling if needed later
    ...
    function renderDiscriminationData(wordsArray, container, knownKeys) {
        ...
        wordsArray.forEach(wordData => {
            ...
            // Section: 通俗描述
            // The wrapper div gets 'section-vernacular-description'
            // The p tag inside will have its font classes toggled by JS
            ...
            if (inReviewMode) {
                if (isVernacular || isSemantic) {
                    ...
                    if (isVernacular) {
                        ...
                        if (pElement) {
                            ...
                            pElement.classList.add('text-lg', 'md:text-xl'); // Increased font size
                        }
                    }
                    // Semantic analysis section remains visible, its collapsibility is handled by user click
                } else {
                ...
                if (isVernacular) {
                    ...
                    if (pElement) {
                        ...
                        pElement.classList.add('text-sm', 'md:text-base'); // Restore original font size
                    }
                }
                // Restore default collapsible state (hidden) for sections that are collapsible, if desired
                // For now, we ensure they are visible. User can re-collapse them.
                // If a section was hidden by review mode, it's made visible.
                // If it was already collapsed by user and *not* hidden by review mode (e.g. semantic analysis), its state is preserved.
            }
        });
    }
