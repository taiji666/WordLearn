...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (collection.sub_groups && collection.sub_groups.length > 0) {
                collection.sub_groups.forEach(subGroup => {
                    ...
                    if (lowerSearchTerm && subGroup.words && subGroup.words.length > 0) {
                        // ✨ 重点修改：增加对 wordObj 和 wordObj.word 类型的检查，确保搜索的健壮性
                        subGroupHasMatchingWord = subGroup.words.some(wordObj => 
                            wordObj && typeof wordObj.word === 'string' && wordObj.word.toLowerCase().includes(lowerSearchTerm)
                        );
                    }
    ...
    if (searchInputElement && groupListElement) {
        searchInputElement.addEventListener('input', (event) => {
            ...
            if (vocabularyDataGlobal) {
                ...
                // Determine current active topic ID based on the page context
                if (wordDisplayAreaElement && selectedGroupTitleElement) { // Index page
                    const urlParams = new URLSearchParams(window.location.search);
                    currentActiveTopicId = urlParams.get('topicId') || (window.history.state ? window.history.state.topicId : (initialTopicIdElement ? initialTopicIdElement.value : null));
                } else if (currentTopicIdDiscriminationPage) { // Discrimination page
        ...
        if (wordDisplayAreaElement && selectedGroupTitleElement) { // Ensure this runs on index.html
            ...
            if (newTopicId && vocabularyDataGlobal) {
                ...
                if (foundSubGroup) {
                    ...
                    // Update active state in sidebar and expand parent
                    document.querySelectorAll('#group-list a').forEach(item => {
                        item.classList.remove('sidebar-link-active');
                        item.classList.add('text-gray-600', 'hover:bg-gray-100', 'hover:text-gray-800');
                         if (item.dataset.subGroupId === newTopicId) {
                            ...
                            const parentLiAncor = item.closest('li');
                            if(parentLiAncor){
                                const subGroupUl = parentLiAncor.parentElement; // ul.subgroup-list
                                if (subGroupUl && subGroupUl.classList.contains('subgroup-list')) {
                                    subGroupUl.classList.remove('hidden');
                                    const collectionButton = subGroupUl.previousElementSibling; // button.collapsible-title
                                    if(collectionButton && collectionButton.matches('button.collapsible-title')) {
                                        const icon = collectionButton.querySelector('.arrow-icon');
    ...
    async function initializePage() {
        ...
        if (data) {
            ...
            if (initialTopicIdElement) { // index.html
                activeTopicId = urlParams.get('topicId') || initialTopicIdElement.value;
            } else if (currentTopicIdDiscriminationPage) { // discrimination_page.html
                activeTopicId = currentTopicIdDiscriminationPage.value || urlParams.get('topicId');
            } else { 
            ...
            if (activeTopicId && wordDisplayAreaElement && selectedGroupTitleElement) { // index.html specific display
                let foundSubGroup = null;
