...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function filterWords(searchTerm) {
        ...
        // Create a clone of current group with filtered words
        const filteredGroup = Object.assign({}, currentGroupData, { words: filteredWords });
        
        // Display filtered words
        displayWords(filteredGroup, true); // true indicates this is filtered view
    }
    ...
    function displayWords(subGroup, isFiltered = false) {
        ...
        // Only update title and currentGroupData when not filtering
        if (!isFiltered) {
            // FIX #1: Check if subGroup exists before accessing its title
            if (subGroup && subGroup.title) {
                selectedGroupTitleElement.textContent = subGroup.title;
                currentGroupData = subGroup;
            } else {
                // Handle case where subGroup is null or missing title
        ...
        // FIX #2: Check if subGroup.words exists
        ...
        // Also store ID for discrimination page even in filtered view
        ...
        loadedVocabularyData.vocabulary_collection.forEach(collection => {
            ...
            if (collection.sub_groups && collection.sub_groups.length > 0) {
                ...
                collection.sub_groups.forEach(subGroup => {
                    ...
                    let displayTitle = subGroup.title;
                    ...
                    // Display subgroup title (removed ID to keep it clean)
                    if (subGroup.words && subGroup.words.length > 0) {
                        subGroupLink.href = `/?topicId=${subGroup.id}`;
                        ...
                        subGroupLink.addEventListener('click', (event) => {
                            if (wordDisplayAreaElement && selectedGroupTitleElement) {
                                ...
                                const selectedSub = parentCol?.sub_groups.find(sg => String(sg.id) === subGroupLink.dataset.subGroupId);
                                if (selectedSub) {
                                    displayWords(selectedSub);
                                    window.history.pushState({ topicId: subGroup.id }, selectedSub.title, `/?topicId=${subGroup.id}`);
