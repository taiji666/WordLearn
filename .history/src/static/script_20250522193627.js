...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayWords(subGroup) {
        ...
        gotoDiscriminationBtn.classList.add('inline-flex'); // For items-center
        gotoDiscriminationBtn.removeEventListener('click', handleGoToDiscrimination);
    ...
    function populateSidebar(loadedVocabularyData, activeTopicId = null) {
        ...
        groupListElement.innerHTML = ''; // Clear current items (e.g., "Loading...")
        ...
        if (gotoDiscriminationBtn) { // Only applies to index.html
            gotoDiscriminationBtn.classList.add('hidden');
            gotoDiscriminationBtn.removeEventListener('click', handleGoToDiscrimination);
        }
            ...
            if (collection.sub_groups && collection.sub_groups.length > 0) {
                ...
                collection.sub_groups.forEach(subGroup => {
                    ...
                    if (subGroup.words && subGroup.words.length > 0) {
                        ...
                        subGroupLink.textContent = subGroup.title;
                        ...
                        subGroupLink.addEventListener('click', (event) => {
                            ...
                            if (wordDisplayAreaElement && selectedGroupTitleElement) { // On index.html
                                ...
                                if (selectedSub) {
                                    ...
                                    window.history.pushState({ topicId: clickedSubGroupId }, selectedSub.title, `/?topicId=${clickedSubGroupId}`); // Update URL
                                    document.querySelectorAll('#group-list a').forEach(item => item.classList.remove('sidebar-link-active', 'hover:bg-gray-100', 'hover:text-gray-800'));
                                    subGroupLink.classList.add('sidebar-link-active');
                                    subGroupLink.classList.remove('hover:bg-gray-100', 'hover:text-gray-800'); // Remove hover styles when active
                                }
                            } else if (currentTopicIdDiscriminationPage) { // On discrimination_page.html
                                if (clickedSubGroupId !== activeTopicId) { // Navigate only if different topic
                ...
                collectionTitleButton.querySelector('.arrow-icon').classList.add('hidden'); // Hide arrow if no subgroups
            }
            groupListElement.appendChild(collectionLi);
        });
    }
    // Handle back/forward navigation for topicId on index.html
    window.addEventListener('popstate', (event) => {
        if (wordDisplayAreaElement && selectedGroupTitleElement) { // Only on index.html
            ...
            if (newTopicId && vocabularyDataGlobal) {
                ...
                if (foundSubGroup) {
                    ...
                    document.querySelectorAll('#group-list a').forEach(item => {
                        ...
                        item.classList.add('text-gray-600', 'hover:bg-gray-100', 'hover:text-gray-800'); // Reset non-active
            ...
            } else if (!newTopicId) { // No topicId, show default
    ...
    async function initializePage() {
        ...
        if (data) {
            ...
            if (initialTopicIdElement && initialTopicIdElement.value) { // From server on index.html
                activeTopicId = initialTopicIdElement.value;
            } else if (currentTopicIdDiscriminationPage && currentTopicIdDiscriminationPage.value) { // From server on discrimination_page.html
                activeTopicId = currentTopicIdDiscriminationPage.value;
            } else { // Fallback to URL param if hidden input is empty (e.g. after client-side nav)
                activeTopicId = urlParams.get('topicId');
            }

            populateSidebar(data, activeTopicId);

            if (activeTopicId && wordDisplayAreaElement && selectedGroupTitleElement) { // On index.html
                let foundSubGroup = null;
                ...
                if (foundSubGroup) {
                    displayWords(foundSubGroup);
                     // Set initial URL for index.html if topicId is present
