document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. 加载辨析 JSON 数据
    fetch('data/vocabulary_discrimination_data.json') // 确保路径正确
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (discriminationData) {
                ...
                contentAreaElement.innerHTML = `<p>未找到ID为 "${topicId}" 的单词背诵信息。</p>`;
                topicTitleElement.textContent = `单词背诵: ${topicId} (未找到)`;
            }
        })
        .catch(error => {
...
function renderDiscriminationData(wordsArray, container) {
    ...
    if (!wordsArray || wordsArray.length === 0) {
        container.innerHTML = '<p>此主题下暂无单词背诵信息。</p>';
        return;
    }

    wordsArray.forEach(wordData => {
        const wordDiv = document.createElement('div');
        wordDiv.className = 'remember-word';

        let htmlContent = `<h2>${wordData.word}</h2>`;
        htmlContent += `<p><span class="main-meaning-title">主要释义:</span> <span class=""main-meaning-content">${wordData['主要释义']}</span></p>`; // 使用方括号访问含中文的键

        if (wordData['全面释义'] && wordData['全面释义'].length > 0) {
            htmlContent += `<p class="all-meaning-title">全面释义:</p><ul class="all-meaning-content">`;
            wordData['全面释义'].forEach(s => { htmlContent += `<li>${s}</li>`; });
            htmlContent += `</ul>`;
        }

        htmlContent += `<p class="discribe-title">通俗描述:</p> <p class="discribe-content">${wordData['通俗描述']}</p>`;
