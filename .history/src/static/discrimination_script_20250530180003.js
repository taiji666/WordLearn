...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleShowWords = document.getElementById('toggle-show-words'); // New toggle from index.html
    const wordsShownCount = document.getElementById('words-shown-count'); // New count span
    ...
    // Variables for new sidebar logic (from script.js)
    let wordContainers = [];
    let totalWordItems = 0;
    // let currentGroupData = null; // May not be needed as extensively as in index.html's main area

    // --- Sidebar Toggle Functionality (from script.js) ---
    ...
    // --- "Show Words List" Toggle Functionality (from script.js) ---
    if (toggleShowWords && wordsShownCount) {
        toggleShowWords.addEventListener('change', function() {
            const isChecked = this.checked;
            localStorage.setItem('showWordsInDiscriminationSidebar', isChecked ? 'true' : 'false'); // Use a different localStorage key
            ...
            document.querySelectorAll('#sidebar .subgroup-arrow').forEach(arrow => {
                ...
                if (subGroupItem) {
                    ...
                    if (wordListContainer) { // Ensure subGroupList exists
    ...
    if (!currentTopicId && topicTitleElement) { // Allow page to load if no topicId for sidebar
        displayError('未指定单词分组ID。无法加载辨析内容。');
        if (topicTitleElement) topicTitleElement.textContent = '单词辨析 (错误)';
        // return; // Don't return, sidebar should still load
    }

    async function fetchDiscriminationData() {
        if (discriminationDataGlobal) return discriminationDataGlobal;
        if (!currentTopicId) return null; // No topic, no specific discrimination data
        try {
    ...
    async function fetchVocabularyData() { // For sidebar (from script.js)
    ...
    // --- Unified populateSidebar function (adapted from script.js) ---
    function populateSidebar(loadedVocabularyData, activeTopicId = null) {
        ...
        loadedVocabularyData.vocabulary_collection.forEach(collection => {
            ...
            if (collection.sub_groups && collection.sub_groups.length > 0) {
                ...
                collection.sub_groups.forEach(subGroup => {
                    ...
                    // Link will navigate for discrimination page
                    ...
                    subGroupLink.addEventListener('click', (event) => {
                        ...
                        if (expandBtn.contains(event.target) && subGroup.words && subGroup.words.length > 0) {
                            ...
                            if (wordListContainer) {
                                ...
                                if (toggleShowWords) { // Sync global toggle if needed
                             ...
                             event.preventDefault(); // Already on this page, prevent full reload, just handle UI
                        }
                        // Navigation for title click is handled by href, unless prevented.
                ...
                collectionTitleButton.querySelector('.arrow-icon').classList.add('hidden'); // No subgroups, no arrow
            }
        ...
        if (wordsShownCount && toggleShowWords) {
            ...
            wordContainers.forEach(container => { // Ensure this uses the populated wordContainers
            ...
            document.querySelectorAll('#sidebar .subgroup-arrow').forEach(arrow => {
                const wordListContainer = arrow.closest('.subgroup-item')?.querySelector('.word-list-container');
                 const hasWords = wordListContainer && wordListContainer.querySelector('.word-card'); // Check if there are actual words
                if (wordListContainer && hasWords) {
                    arrow.classList.toggle('expanded', showWords && !wordListContainer.classList.contains('hidden'));
                } else if (!hasWords) {
                     arrow.classList.remove('expanded');
                     arrow.classList.add('opacity-0', 'pointer-events-none'); // Hide if no words
                }
            });
        }
    }

    // --- Sidebar Search Functionality (adapted from script.js) ---
    if (sidebarSearchInput && groupListElement) {
        sidebarSearchInput.addEventListener('input', function() {
            const searchTerm = this.value.trim().toLowerCase();
            
            if (!searchTerm) {
                // Re-initialize sidebar to normal state if vocabularyDataGlobal exists
                if (vocabularyDataGlobal) populateSidebar(vocabularyDataGlobal, currentTopicId);
                return;
            }
            
            const searchResultsContainer = document.createElement('div');
            searchResultsContainer.id = 'search-results-container'; // For potential styling from index.html
            searchResultsContainer.className = 'space-y-2';
            groupListElement.innerHTML = ''; // Clear current list
            ...
            vocabularyDataGlobal.vocabulary_collection.forEach(collection => {
                ...
                collection.sub_groups.forEach(subGroup => {
                    ...
                    if (groupMatched) {
                        ...
                        groupLink.href = `/discrimination?topicId=${subGroup.id}`;
                        groupLink.addEventListener('click', () => { // For SPA-like feel on mobile if needed
                             if (window.innerWidth < 1024 && sidebar && !sidebar.classList.contains('-translate-x-full') ) {
                                sidebar.classList.remove('translate-x-0');
                                sidebar.classList.add('-translate-x-full');
                                if(sidebarToggle) sidebarToggle.classList.remove('hidden');
                            }
                            // Actual navigation is by href
                        });
                        ...
                        matches.forEach(match => {
                            ...
                            wordLink.href = `/discrimination?topicId=${match.subGroupId}`; // Navigate to word's group discrimination
            ...
            if (!anyMatchFound) {
                ...
                infoDiv.innerHTML = `找到 ${resultCount} 个匹配的单词`; // Only counts words, not groups
    ...
    async function initializeDiscriminationPage() {
        // 1. Fetch vocabulary data for the sidebar first
        const vocabularyData = await fetchVocabularyData();
        if (vocabularyData) {
            populateSidebar(vocabularyData, currentTopicId); // Pass currentTopicId to highlight active
        } else {
            // Handle case where sidebar vocab fails but page might still show discrimination if currentTopicId is valid
             if (groupListElement) groupListElement.innerHTML = `<li class="p-3 text-sm text-red-500">侧边栏加载失败。</li>`;
        }

        // 2. If currentTopicId is present, fetch and render discrimination data for main content
        if (currentTopicId) {
            const discriminationData = await fetchDiscriminationData();
            if (!discriminationData) { // Error handled by fetchDiscriminationData
                 if (!contentAreaElement.innerHTML.includes('alert')) { // Avoid double error messages
            ...
            if (discriminationGroupData) {
                ...
                initializeInteractiveElements(); // For main content collapsibles
            } else {
            ...
            // No currentTopicId, main area shows a generic message
    ...
    function initializeInteractiveElements() { // For main content collapsibles
        ...
        contentAreaElement.addEventListener('click', function(event) {
            ...
            if (toggle) {
                ...
                if (contentWrapper && contentWrapper.classList.contains('mt-2')) { 
                    ...
                    toggle.classList.toggle('open'); // For main content collapsible arrow
